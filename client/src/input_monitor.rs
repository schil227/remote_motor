...
use crate::motor_message_constatnts::MotorMessageConstants;
    ...
    loop
    {
        match read().unwrap(){
            ...
            Event::Key(KeyEvent { code: KeyCode::Char('m'), modifiers: _ }) =>{
                ...
                let mut message = shared_command.lock().unwrap();
                (*message) = MotorMessageConstants::rotate_shoulder_clockwise();
            },
            Event::Key(KeyEvent { code: KeyCode::Char('.'), modifiers: _ }) =>{
                println!("Rotating Shoulder Counter-Clockwise!");
                let mut message = shared_command.lock().unwrap();
                (*message) = MotorMessageConstants::rotate_shoulder_counter_clockwise();
            },
            Event::Key(KeyEvent { code: KeyCode::Char(','), modifiers: _ }) =>{
                println!("Stopping Shoulder!");
                let mut message = shared_command.lock().unwrap();
                (*message) = MotorMessageConstants::stop_shoulder();
            }, 
            ...
            _ => {
                ...
                let mut message = shared_command.lock().unwrap();
                (*message) = MotorMessageConstants::stop_everything();
            }
        }
    }
}